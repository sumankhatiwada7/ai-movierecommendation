...
export default function BrowseMovies() {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetchmovies({ page, limit: 20, genreId: genreId || undefined, sortBy })
      .then((data) => {
    ...
    <div className="bg-black min-h-screen text-white p-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {movies.map((movie) => (
            <Link to={`/movies/${movie.id}`} key={movie.id} className="hover:scale-105 transition-transform">
              {movie.posterUrl ? (
                <img src={movie.posterUrl} alt={movie.title} className="rounded w-full h-60 object-cover" />
              ) : (
